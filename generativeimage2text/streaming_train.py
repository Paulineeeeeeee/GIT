...
def mytrain(param,args):
    ...
    annotations_file = '/local/pauline/GIT/preprocessing/no_miss_install_train.jsonl'
    data_path = '/home/pauline/no-miss-AITW/install/'
    ...
    # load data
    TrainDataset = LlamaTouch('TRAIN',tokenizer,transform = trsfm(image_size=image_size,split='TRAIN'),image_encoder = image_encoder,\
        num_images=param['num_image_with_embedding'])
    ...
    ValidDataset = LlamaTouch(data_path_V2,'VALID',tokenizer,transform = trsfm(image_size=image_size,split='VALID'),image_encoder = image_encoder,\
        num_images=param['num_image_with_embedding'])
